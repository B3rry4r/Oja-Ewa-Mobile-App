...
- **Node.js AI Backend:** `https://ojaewa-ai-production-1bb8.up.railway.app` (New service you're building)
...
const [laravelData, aiData] = await Promise.all([
  ...
  fetch('https://ojaewa-ai-production-1bb8.up.railway.app/ai/recommendations/123', {
    headers: { 'Authorization': `Bearer ${token}` }
  })
]);

// Option 2: AI service calls Laravel internally
const aiData = await fetch('https://ojaewa-ai-production-1bb8.up.railway.app/ai/recommendations/123', {
  headers: { 'Authorization': `Bearer ${token}` }
});
